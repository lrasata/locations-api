...
data "aws_secretsmanager_secret_version" "geo_secret_key" {
...
resource "aws_lambda_function" "weather_api" {
  ...
  environment {
    variables = {
      ...
      GEO_DB_RAPID_API_KEY     = "${data.aws_secretsmanager_secret.geo_db_rapid_api_secrets.arn}:GEO_DB_RAPID_API_KEY::"
    }
  }
}
